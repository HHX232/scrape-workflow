...
export async function DownloadPdfExecutor(enviroment: ExecutionEnviroment<typeof DownloadPdf>): Promise<boolean> {
  try {
    ...
    enviroment.log.info(`PDF downloaded successfully, size: ${(response.data.byteLength / 1024).toFixed(2)} KB`)
    ...
    if (axios.isAxiosError(error)) {
      if (error.code === 'ECONNABORTED') {
        ...
        enviroment.log.error(`Failed to download PDF: HTTP ${error.response.status} ${error.response.statusText}`)
